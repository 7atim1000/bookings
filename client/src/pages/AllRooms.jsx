...
const AllRooms = () => {
    ...
    // md:px-16 px-4 lg:px-24 xl:px-32
   ...
   const handleSortChange = (sortOption) => {
    ...
    // function to check if room matches the selected prcice ranges
    ...
    }, [rooms, selectedFilters, selectedSort, searchParams ,matchesRoomType, matchesPriceRange, filterDestination, sortRooms])
    ...
    return(
       <div className ='flex flex-col lg:flex-row items-start justify-between pt-28 md:pt-20 px-4 md:px-10 lg:px-24 xl:px-24'>
           <div>
                <div className ='flex flex-col items-start text-left'>
                    <h1 className ='font-playfair text-4xl md:text-[40px]'>Hotels Rooms</h1>
                    <p className ='text-sm md:text-base text-gray-500/90 mt-2 max-w-174'>Take advantage of our limited-time offers
                    and special packages to enhance your stay and create unforgettable memories.
                    </p>
                </div>

                {filteredRooms.map((room)=>(
                    <div key={room._id} className ='mt-10 flex flex-col md:flex-row items-start py-10 gap-6 border-b border-gray-200 last:pb-30
                    last: border-0'>
                        <img  
                        ...
                        <div className ='md:w-1/2 flex flex-col gap-2'>
                            ...
                            <div className ='text-gray-500 mt-2 text-sm flex items-center gap-1'>
                                <MdLocationPin  className ='inline w-6 h-6 text-green-600'/>
                            ...
                            {/*Price per night */}
                            <p className ='text-sm text-stone-400'>AED <span className ='text-lg text-stone-800'>{room.pricePerNight}</span> / night</p>
                        </div>
                    </div>
                ))}
           </div>
           ...
           {/*Filter */}
            <div className='bg-white w-70 border border-gray-300 text-gray-600 max-lg:mb-8 min-lg:mt-15'>
                
                <div className ={`flex items-center justify-between px-5 py-2.5 min-lg:border-b border-gray-300
                    min-lg:sticky ${openFilter && 'border-b'}`}>
                    <p className ='text-base text-md font-medium text-gray-800'>FILTER</p>
                    ...
                    <div className ='px-5 pt-5'>
                        <p className ='font-medium text-gray-800 pb-2'>Price Ranges</p>
                        {
                            priceRanges.map((range, index)=>(
                                <CheckBox key={index} label ={`AED ${range}`}
